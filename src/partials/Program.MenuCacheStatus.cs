...
{
    // Colors only when the console can process them; otherwise plain text (no leaked escape codes).
    private static readonly bool MenuCacheColorEnabled = !Console.IsOutputRedirected && WindowsVirtualTerminal.TryEnable();
    ...
    internal static string GetPlatformLimitHeadroomText()
    {
        ...
        if (status.IsOverLimit)
        {
            return $"{MenuCacheLabel}: EXCEEDED";
        }

        int percent = (int)Math.Round(Math.Clamp(status.Fraction, 0, 1) * 100);
        string headroom = $"~{status.RemainingPlatforms} platform{(status.RemainingPlatforms == 1 ? "" : "s")} left";

        return $"{MenuCacheLabel}: {percent}% ({headroom})";
    }
