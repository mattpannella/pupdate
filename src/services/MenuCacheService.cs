...
public class MenuCacheService
{
    ...
    // ===== headroom status (read-only, for the UI indicator) =====

    // Approx bytes one more single-core platform adds to the view (116-byte record + 4 first membership).
    public const int ApproxPlatformCost = 120;

    public enum MenuCacheLevel { Safe, Close, Danger, Over }

    public class MenuCacheStatus
    {
        public int ProjectedSize { get; set; }
        public int BufferSize { get; set; }
        public int RemainingBytes { get; set; }        // negative when over
        public int RemainingPlatforms { get; set; }    // approx additional platforms that still fit
        public int InstalledPlatforms { get; set; }
        public int Cores { get; set; }
        public double Fraction { get; set; }           // ProjectedSize / BufferSize, may exceed 1.0
        public MenuCacheLevel Level { get; set; }
        public bool IsOverLimit { get; set; }
    }

    // Reads /Cores + /Platforms (cached per instance); reuse the snapshot rather than calling per redraw.
    public MenuCacheStatus GetStatus()
    {
        CacheModel model = this.BuildModel();
        int projected = this.ProjectedCoreViewSize();
        int remainingBytes = BufferSize - projected;
        int remainingPlatforms = Math.Max(0, remainingBytes / ApproxPlatformCost);
        double fraction = (double)projected / BufferSize;

        MenuCacheLevel level =
            projected >= BufferSize ? MenuCacheLevel.Over
            : remainingPlatforms <= 2 ? MenuCacheLevel.Danger
            : remainingPlatforms <= 20 ? MenuCacheLevel.Close
            : MenuCacheLevel.Safe;

        return new MenuCacheStatus
        {
            ProjectedSize = projected,
            BufferSize = BufferSize,
            RemainingBytes = remainingBytes,
            RemainingPlatforms = remainingPlatforms,
            InstalledPlatforms = model.Platforms.Count(p => p.Installed),
            Cores = model.Cores.Count,
            Fraction = fraction,
            Level = level,
            IsOverLimit = projected >= BufferSize
        };
    }
