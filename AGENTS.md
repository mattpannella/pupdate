...
A full-screen Terminal.Gui v2 (`Terminal.Gui` 2.4.x, net10-only) interface, marked **Beta**. It's opt-in: the classic menu is the default, and users reach the TUI via a one-time startup prompt (remembered in the `use_tui` setting, also a toggle in the settings menu / Settings tab), the `--tui` flag, or `PUPDATE_TUI=1`; `--classic` forces the classic menu and skips the prompt. Before the TUI is chosen, `WindowsVirtualTerminal.TryEnable` turns on ANSI/VT processing — the legacy Windows console host (conhost, e.g. double-clicking pupdate.exe) leaves it off and Terminal.Gui renders blank (issue #490); if it can't be enabled the launcher falls back to the classic menu, and `TuiApp.Run` restores the original console mode on exit. It calls the **same services** as the classic menu — only the presentation differs. Every service-layer change must stay backward-compatible so the classic console UI keeps working.
...
- **`TuiHost`** — the single seam over Terminal.Gui's application loop: holds the one `IApplication` instance (`Application.Create()`) so no other TUI file touches it. Use `TuiHost.Init/Run/Shutdown/Invoke/RequestStop/Refresh/AddTimeout/RemoveTimeout` rather than reaching for `Application`/`App` directly.
- **`TuiShell`** — root window: pinned welcome banner, a `Tabs` strip (Main / Setup / Maintenance / Extras / Plugins / Settings), the `StatusPane`, and a bottom `StatusBar` of `Shortcut` key hints (Esc quit · F6 status pane · F9 theme). The StatusBar is **display + click only** — `BindKeyToApplication` stays false so the actual keys ride the bubble-up `Window.KeyDown` (no view consumes function keys), sharing the same private methods as the Shortcut click actions — plus the single-key accelerators below, which are handled in `OnGlobalKeyDown`. Keep tabs/status as direct children of the window (a non-focusable container breaks tab navigation).
- **`TuiContext.RunBackground`** — runs blocking service calls on a `Task` (single-operation busy guard) so the UI never freezes. `BusyChanged` also drives `StatusPane.SetBusy`: a `SpinnerView` + marquee-style progress bar run while an operation has no download progress to show, and the first real `SetProgress` switches the bar back to true progress (solid `Continuous` fill; percentage + speed in the label beside it).
...
- Dialogs (`src/tui/Dialogs/`): `ChecklistDialog` (marking list + type-ahead filter + optional `DropDownList` category filter), `SelectDialog`, `CoreSelectorDialog`, `DisplayModeSelectorDialog`, `SubMenuDialog` (groups actions behind one `…` entry), `PluginRunModal`, `SupportModal` (sponsor URLs render as clickable `Link` views), `ThemePickerDialog`; plus `TuiPrompts` (Confirm / PromptText). The old "button-bar steals focus from a `TextField`" bug is fixed in 2.4.12 — text fields coexist with dialog buttons (PromptText has Save/Cancel). **Dialog layout landmine:** `AddButton` reserves the bottom of the dialog by *shrinking the content area* (`GetContentSize()`), and any subview positioned below it is silently clipped while staying focusable — an "invisible but typable" field. Keep fixed-Y content within the content rows (or use `Dim.Fill`-relative sizing), and verify with a screen dump if in doubt. `DropDownList` gotcha: when closed it binds ↑/↓ to cycling its own selection, trapping the cursor — `ChecklistDialog` removes those two key bindings and, after a pick, returns focus to the list via a zero-delay `TuiHost.AddTimeout` (the closing popover re-focuses the dropdown *after* `ValueChanged`, so a plain `Invoke` runs too early).
- `AnalogizerWizard`: both flows share one wizard page shape (`BuildStep`): a prompt `Label` above an `OptionSelector` radio list — click/Space only marks the choice (`DoubleClickAccepts = false`), Back/Next (or Enter) navigate. Standard is one multi-step `Wizard` (the SNAC-assignment step is skipped by disabling it — the wizard auto-skips disabled steps; `Dialog.Canceled` after `TuiHost.Run` distinguishes Finish from Esc). Jotego is service-driven one callback at a time, so each prompt is its own single-page wizard. Put step prompts in a `Label` above the content, not `WizardStep.HelpText` — the help text renders in a detached pane on the right, which reads as unrelated to the options.
