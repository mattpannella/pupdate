...
- **`TuiContext.RunBackground`** — runs blocking service calls on a `Task` (single-operation busy guard) so the UI never freezes. `BusyChanged` also drives `StatusPane.SetBusy`: a `SpinnerView` + marquee-style progress bar run while an operation has no download progress to show, and the first real `SetProgress` switches the bar back to a true percentage (rendered by the bar itself via `ProgressBarFormat.SimplePlusPercentage`; the label next to it shows only the speed).
